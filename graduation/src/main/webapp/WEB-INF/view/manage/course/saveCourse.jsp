...
<form class="layui-form layui-form-pane" action="" style="width:80%;margin:0 auto;margin-top: 2%;"
    ...
    <div class="layui-form-item">
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn" lay-submit="" lay-filter="saveCourse">立即保存</button>
...
<script type="text/javascript">
    ...
    layui.config({
        ...
        form.on("submit(saveCourse)", function (data) {
            ...
            var msg;
            var subURL = ctx + "/manage/saveCourse";//添加提交地址
            if (!isEmpty(data.field.id)) { //判断是添加还是修改
                subURL = ctx + "/manage/updateCourse";//修改提交地址
            }
            $.ajax({
                type: "post",
                url: subURL,
                ...
                success: function (d) {
                    if (d.code == 0) {
                        msg = d.msg;
                        // 重置表单 saveCourseForm是表单的id
                        //$("#saveCourseForm")[0].reset();
                        //layui.form.render();
                        layer.msg(msg, {time: 3000, icon: 1}, function () {
                            var url = ctx + "/manage/courseList"; //返回列表页面
