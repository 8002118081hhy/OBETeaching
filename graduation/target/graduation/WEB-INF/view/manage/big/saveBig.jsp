...
<form class="layui-form layui-form-pane" action="" style="width:80%;margin:0 auto;margin-top: 2%;"
    ...
    <input type="hidden" value="${big.id}" name="id">
                            <div class="layui-form-item ">
    <label class="layui-form-label">名称</label>
    <div class="layui-input-block">
                <input type="text" id="name" name="name" lay-verify="required"
               placeholder="请输入名称"
                              value="${big.name}" class="layui-input"/>
            </div>
    </div>
    ...
    <div class="layui-form-item">
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn" lay-submit="" lay-filter="saveBig">立即保存</button>
...
<script type="text/javascript">
    ...
    layui.config({
        ...
        //表单提交
        form.on("submit(saveBig)", function (data) {
            //弹出loading
            var index = top.layer.msg('数据提交中，请稍候', {icon: 16, time: false, shade: 0.8});
            var msg;
            var subURL = ctx + "/manage/saveBig";//添加提交地址
            if (!isEmpty(data.field.id)) { //判断是添加还是修改
                subURL = ctx + "/manage/updateBig";//修改提交地址
            }
            $.ajax({
                type: "post",
                url: subURL,
                ...
                success: function (d) {
                    if (d.code == 0) {
                        msg = d.msg;
                        // 重置表单 saveBigForm是表单的id
                        //$("#saveBigForm")[0].reset();
                        //layui.form.render();
                        layer.msg(msg, {time: 3000, icon: 1}, function () {
                            var url = ctx + "/manage/bigList"; //返回列表页面
